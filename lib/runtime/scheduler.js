...
export function schedule(task) {
  if (currentTask) {
    if (__DEBUG__) {
      console.debug(
        `[[Schedule]] ${task.initiator.type.name} (${task.id}) waiting...`,
        task
      );
    ...
    if (__DEBUG__) {
      console.debug(
        `[[Schedule]] ${task.initiator.type.name} (${task.id}) starting...`,
        task
      );
    }
    currentTask = task;
...
function doWork(deadline, input = null) {
  if (__DEBUG__) {
    console.debug(
      `[[Schedule]] ${currentTask.initiator.type.name} (${currentTask.id}) continue:`,
  ...
  do {
    ...
    if (done) {
      if (__DEBUG__) {
        console.debug(
          `[[Schedule]] ${currentTask.initiator.type.name} (${currentTask.id}) ready to commit`
        );
      }
...
function commit() {
  ...
  if (__DEBUG__) {
    console.debug(
      `[[Schedule]] ${currentTask.initiator.type.name} (${currentTask.id}) committed`
    );
    // debugger;
  }
  currentTask.effects = [];
